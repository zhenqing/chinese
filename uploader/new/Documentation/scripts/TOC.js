...
function Toggle(node)
{
    ...
    {
        div.className = "Hidden";
        node.src = "images/Collapsed.gif";
    }
    else
    {
        div.className = "Visible";
        node.src = "images/Expanded.gif";
...
function scrollToAnchor(iframe)
{
	...
	highlightNode(url);
	modifyLinks(iframe.contentWindow.document.links, url);
	...
	for(var i = 0; i < iframe.contentWindow.document.anchors.length; i++)
	{
		linkObj = iframe.contentWindow.document.anchors[i];
		
		if(linkObj.name == id)
		{
			linkObj.scrollIntoView(true);
			break;
		}
	}	
...
var parrentsArray = new Array();
function highlightNode(currentUrl)
{
	...
	if(document.all)
	{
		currentUrl = currentUrl.substring(path.length);			
	}
	else
	{
		currentUrl = currentUrl.substring(path.length);
	...
	for(var i = 0; i < document.links.length; i++)
	{
		...
		hrefPattern = unescape(linkObj.href).substring(path.length);
				
		if(hrefPattern == currentUrl)
		{
			isTreeLink = true;
			break;
		}
	...
	if(linkObj.className == "SelectedNode")
	{
		return;
	}
	
	parrentsArray = new Array();
	getParrents(linkObj.parentNode);
	
	
	for(var j = parrentsArray.length - 1; j >= 0; j--)
	{
		Expand(parrentsArray[j]);	
	}
	
	SelectNode(linkObj);
}

function getParrents(parentNode)
{
	try
	{
		if(parentNode.className == "Tree")
			return;
			
		if(parentNode.className == "TreeNode")
			parrentsArray.push(parentNode);
			
		getParrents(parentNode.parentNode, parrentsArray);
	}
	catch(ex)
	{
		alert(ex.message);	
...
function modifyLinks(linksArray, currentFrameUrl)
{
	currentFrameUrl = unescape(currentFrameUrl);
	
	if(currentFrameUrl.indexOf("#") != -1)
		currentFrameUrl = currentFrameUrl.substring(0, currentFrameUrl.indexOf("#"));
		
	var hrefPattern;
	for(var i = 0; i < linksArray.length; i++)
	{
		hrefPattern = unescape(linksArray[i].href);
		
		if(hrefPattern.indexOf(currentFrameUrl) == 0)
		{
			linksArray[i].onclick = anchorClickHandler;
		}
	}	
	
}

function anchorClickHandler(eventObj)
{	
	var evt = topicContent.contentWindow.event || eventObj;
	var linkObj = evt.srcElement || evt.target;
	
	highlightNode(linkObj.href);	
	return true;
}